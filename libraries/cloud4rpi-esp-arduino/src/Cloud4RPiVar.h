...
template <typename T> class C4RVariable : public C4RVariableBase
{
  private:
    String type;
    T value;
  ...
  public:
    C4RVariable(const String &_name, const String &_type, T _value)
        : C4RVariableBase(_name), type(_type), value(_value) {};
...
class C4RVariableStorage
{
  ...
  public:
    ...
    template <typename T> T getValue(const String &varName)
    {
        C4RVariable<T> *var = (C4RVariable<T> *)list->find(varName);
        return var->getValue();
    }

    template <typename T> void setValue(const String &varName, T _value)
    {
        C4RVariable<T> *var = (C4RVariable<T> *)list->find(varName);
        if (var)
        {
    ...
    C4RVariableBase *find(const String &name) { return list->find(name); }

    template <typename T> void handleCommand(const String &command, T _value)
    { // optimize double find
