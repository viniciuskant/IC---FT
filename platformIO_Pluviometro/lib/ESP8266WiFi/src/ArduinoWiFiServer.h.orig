...
{
  public:
    ArduinoCompatibleWiFiServerTemplate(const IPAddress &addr, uint16_t port)
        : TServer(addr, port)
    {
    }
        ...
        while (true)
        {
            for (uint8_t i = 0; i < MAX_MONITORED_CLIENTS; i++)
            {
                ...
                if (client.status() == ESTABLISHED &&
                    client.availableForWrite() < (int)a)
                {
    ...
    virtual void flush() override { flush(0); }
    ...
    operator bool() { return (TServer::status() == LISTEN); }
...
typedef ArduinoCompatibleWiFiServerTemplate<WiFiServer, WiFiClient>
    ArduinoWiFiServer;
typedef ArduinoCompatibleWiFiServerTemplate<WiFiServerSecure, WiFiClientSecure>
    ArduinoWiFiServerSecure;
