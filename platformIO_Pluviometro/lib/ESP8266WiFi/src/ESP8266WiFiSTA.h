...
class ESP8266WiFiSTAClass : public LwipIntf
{
  ...
  public:
    ...
    wl_status_t begin(const String &ssid,
                      const String &passphrase = emptyString,
                      int32_t channel = 0, const uint8_t *bssid = NULL,
                      bool connect = true);
    wl_status_t begin();

    // The argument order for ESP is not the same as for Arduino. However, there
    // is compatibility code under the hood to detect Arduino arg order, and
    // handle it correctly. Be aware that the Arduino default value handling
    // doesn't work here (see Arduino docs for gway/subnet defaults). In other
    // words: at least 3 args must always be given.
