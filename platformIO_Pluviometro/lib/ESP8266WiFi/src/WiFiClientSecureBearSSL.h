...
class WiFiClientSecureCtx : public WiFiClient
{
  public:
    ...
    // TODO: usage is invalid b/c of deleted copy, but this will only trigger an
    // error when it is actually used by something
    // TODO: don't remove just yet to avoid including the WiFiClient default
    // implementation and unintentionally causing
    //       a 'slice' that this method tries to avoid in the first place
    std::unique_ptr<WiFiClient> clone() const override { return nullptr; }
    ...
    void setKnownKey(const PublicKey *pk,
                     unsigned usages = BR_KEYTYPE_KEYX | BR_KEYTYPE_SIGN)
    {
        _clearAuthenticationSettings();
        _knownkey = pk;
    ...
    // In cases when NTP is not used, app must set a time manually to check cert
    // validity
    void setX509Time(time_t now) { _now = now; }
    // Install a client certificate for this connection, in case the server
    // requires it (i.e. MQTT)
    ...
    // Returns whether MFLN negotiation for the above buffer sizes succeeded
    // (after connection)
    int getMFLNStatus()
    {
    ...
    // Return an error code and possibly a text string in a passed-in buffer
    // with last SSL failure
    ...
    bool setCiphersLessSecure(); // Only use the limited set of RSA ciphers
    // without EC
  ...
  private:
    ...
    br_ssl_engine_context
        *_eng; // &_sc->eng, to allow for client or server contexts
    ...
    int _pollRecvBuffer(); // If there's a buffer with some pending data, return
    // it's length If there's no buffer, poll the engine
    // and store any received data there and return the
    // length (which also may change the internal state,
    // e.g. make us disconnected)
    ...
    bool _wait_for_handshake(); // Sets and return the _handshake_done after
    // connecting
    ...
    bool
    _installClientX509Validator(); // Set up X509 validator for a client conn.
    bool _installServerX509Validator(
        const X509List
            *client_CA_ta); // Setup X509 client cert validation, if supplied
...
class WiFiClientSecure : public WiFiClient
{
    ...
    // Instead, all virtual functions call their counterpart in
    // "WiFiClientecureCtx* _ctx"
    //          which also derives from WiFiClient (this parent is the one which
    //          is eventually used)

    // TODO: notice that this complicates the implementation by having two
    // distinct ways the client connection is managed, consider:
    // - implementing the secure connection details in the ClientContext
    //   (i.e. delegate the write & read functions there)
    // - simplify the inheritance chain by implementing base wificlient class
    // and inherit the original wificlient and wificlientsecure from it
    // - abstract internals so it's possible to seamlessly =default copy and
    // move with the instance *without* resorting to manual copy and
    // initialization of each member

    // TODO: prefer implementing virtual overrides in the .cpp (or, at least one
    // of them)
  ...
  public:
    ...
    {
        return _ctx->setCiphersLessSecure();
    } // Only use the limited set of RSA ciphers without EC
    ...
    // Check for Maximum Fragment Length support for given len before connection
    // (possibly insecure)
    static bool probeMaxFragmentLength(IPAddress ip, uint16_t port,
                                       uint16_t len);
    static bool probeMaxFragmentLength(const char *hostname, uint16_t port,
                                       uint16_t len);
    static bool probeMaxFragmentLength(const String &host, uint16_t port,
                                       uint16_t len);
  ...
  private:
    ...
    WiFiClientSecure(ClientContext *client, const X509List *chain,
                     unsigned cert_issuer_key_type, const PrivateKey *sk,
                     int iobuf_in_size, int iobuf_out_size,
                     ServerSessions *cache, const X509List *client_CA_ta,
                     int tls_min, int tls_max)
        : _ctx(new WiFiClientSecureCtx(client, chain, cert_issuer_key_type, sk,
                                       iobuf_in_size, iobuf_out_size, cache,
                                       client_CA_ta, tls_min, tls_max))
    {
    }
    ...
    WiFiClientSecure(ClientContext *client, const X509List *chain,
                     ...
                     int iobuf_out_size, ServerSessions *cache,
                     const X509List *client_CA_ta, int tls_min, int tls_max)
        : _ctx(new WiFiClientSecureCtx(client, chain, sk, iobuf_in_size,
                                       iobuf_out_size, cache, client_CA_ta,
                                       tls_min, tls_max))
    {
    }
