...
// Holds either a single public RSA or EC key for use when BearSSL wants a
// pubkey. Copies all associated data so no need to keep input PEM/DER keys. All
...
// Holds either a single private RSA or EC key for use when BearSSL wants a
// secretkey. Copies all associated data so no need to keep input PEM/DER keys.
...
// Opaque object which wraps the BearSSL SSL session to make repeated
// connections significantly faster.  Completely optional.
...
class ServerSessions
{
  ...
  public:
    // Uses the given buffer to cache the given number of sessions and
    // initializes it.
    ServerSessions(ServerSession *sessions, uint32_t size)
        : ServerSessions(sessions, size, false)
    {
    }

    // Dynamically allocates a cache for the given number of sessions and
    // initializes it. If the allocation of the buffer wasn't successful, the
    // value returned by size() will be 0.
    ServerSessions(uint32_t size)
        : ServerSessions(size > 0 ? new ServerSession[size] : nullptr, size,
                         true)
    {
    }
...
class SigningVerifier : public UpdaterVerifyClass
{
  public:
    ...
    virtual bool verify(UpdaterHashClass *hash, const void *signature,
                        uint32_t signatureLen) override;
...
extern "C"
{
    ...
    thunk_br_ssl_engine_recvapp_buf(const br_ssl_engine_context *cc,
                                    size_t *len);
    extern void thunk_br_ssl_engine_recvapp_ack(br_ssl_engine_context *cc,
                                                size_t len);
    extern unsigned char *
    thunk_br_ssl_engine_recvrec_buf(const br_ssl_engine_context *cc,
                                    size_t *len);
    extern void thunk_br_ssl_engine_recvrec_ack(br_ssl_engine_context *cc,
                                                size_t len);
    extern unsigned char *
    thunk_br_ssl_engine_sendapp_buf(const br_ssl_engine_context *cc,
                                    size_t *len);
    extern void thunk_br_ssl_engine_sendapp_ack(br_ssl_engine_context *cc,
                                                size_t len);
    extern unsigned char *
    thunk_br_ssl_engine_sendrec_buf(const br_ssl_engine_context *cc,
                                    size_t *len);
