...
class WiFiServerSecure : public WiFiServer
{
  public:
    ...
    void setCache(ServerSessions *cache) { _cache = cache; }

    // Set the server's RSA key and x509 certificate (required, pick one).
    // Caller needs to preserve the chain and key throughout the life of the
    // server.
    void setRSACert(const X509List *chain, const PrivateKey *sk);
    // Set the server's EC key and x509 certificate (required, pick one)
    // Caller needs to preserve the chain and key throughout the life of the
    // server.
    void setECCert(const X509List *chain, unsigned cert_issuer_key_type,
                   const PrivateKey *sk);

    // Require client certificates validated against the passed in x509 trust
    // anchor Caller needs to preserve the cert throughout the life of the
    // server.
    ...
    // If awaiting connection available and authenticated (i.e. client cert),
    // return it.
    WiFiClientSecure
    accept(); // https://www.arduino.cc/en/Reference/EthernetServerAccept
    WiFiClientSecure available(uint8_t *status = NULL)
        __attribute__((deprecated("Renamed to accept().")));
