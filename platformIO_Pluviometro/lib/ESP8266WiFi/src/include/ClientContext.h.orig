...
class ClientContext
{
  public:
    ClientContext(tcp_pcb *pcb, discard_cb_t discard_cb, void *discard_cb_arg)
        : _pcb(pcb), _rx_buf(0), _rx_buf_offset(0), _discard_cb(discard_cb),
          _discard_cb_arg(discard_cb_arg), _refcnt(0), _next(0),
    ...
    size_t availableForWrite() const { return _pcb ? tcp_sndbuf(_pcb) : 0; }
    ...
    size_t read(char *dst, size_t size)
    {
        ...
        while (size)
        {
            ...
            DEBUGV(":rdi %d, %d\r\n", buf_size, copy_size);
            os_memcpy(dst,
                      reinterpret_cast<char *>(_rx_buf->payload) +
                          _rx_buf_offset,
                      copy_size);
            dst += copy_size;
            _consume(copy_size);
    ...
    size_t peekBytes(char *dst, size_t size) const
    {
        ...
        DEBUGV(":rpi %d, %d\r\n", buf_size, copy_size);
        os_memcpy(dst,
                  reinterpret_cast<char *>(_rx_buf->payload) + _rx_buf_offset,
                  copy_size);
        return copy_size;
    }
    ...
    bool wait_until_acked(int max_wait_ms = WIFICLIENT_MAX_FLUSH_WAIT_MS)
    {
        ...
        // option 1 done
        // option 2 / _write_some() not necessary since _datasource is always
        // nullptr here
  ...
  protected:
    bool _is_timeout() { return millis() - _op_start_time > _timeout_ms; }
    ...
    size_t _write_from_source(const char *ds, const size_t dl)
    {
        ...
        do
        {
            ...
            _send_waiting = true;
            // will resume on timeout or when _write_some_from_cb or
            // _notify_error fires
    ...
    bool _write_some()
    {
        ...
        while (_written < _datalen)
        {
            ...
            size_t next_chunk_size =
                std::min((size_t)tcp_sndbuf(_pcb), remaining);
            if (!next_chunk_size)
                break;
            ...
            if (next_chunk_size < remaining)
                //   PUSH is meant for peer, telling to give data to user app as
                //   soon as received PUSH "may be set" when sender has finished
                //   sending a "meaningful" data block PUSH does not break Nagle
            ...
            if (!_sync)
                // user data must be copied when data are sent but not yet
                // acknowledged (with sync, we wait for acknowledgment before
                // returning to user)
        ...
        if (has_written)
        {
            ...
            // more info:
            // https://lists.gnu.org/archive/html/lwip-users/2017-11/msg00134.html
            tcp_output(_pcb);
        }
    ...
    static err_t _s_recv(void *arg, struct tcp_pcb *tpcb, struct pbuf *pb,
                         err_t err)
    {
