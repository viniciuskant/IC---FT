...
class UdpContext
{
  public:
    ...
    UdpContext()
        : _pcb(0), _rx_buf(0), _first_buf_taken(false), _rx_buf_offset(0),
          _rx_buf_size(0), _refcnt(0), _tx_buf_head(0), _tx_buf_cur(0),
          _tx_buf_offset(0)
    {
        _pcb = udp_new();
    ...
    bool connect(const IPAddress &addr, uint16_t port)
    {
        ...
        if (IP_IS_V6(&_pcb->remote_ip) &&
            ip6_addr_lacks_zone(ip_2_ip6(&_pcb->remote_ip), IP6_UNKNOWN))
        {
            ip6_addr_assign_zone(ip_2_ip6(&_pcb->remote_ip), IP6_UNKNOWN,
                                 netif_default);
        }
#endif
        return true;
    ...
    void disconnect() { udp_disconnect(_pcb); }
    ...
    void setMulticastInterface(netif *p_pNetIf)
    {
        udp_set_multicast_netif_index(
            _pcb, (p_pNetIf ? netif_get_index(p_pNetIf) : NETIF_NO_INDEX));
    }

    /*
     * Allow access to pcb to change eg. options
     */
    udp_pcb *pcb(void) { return _pcb; }
    ...
    void printChain(const pbuf *pb, const char *msg, size_t n) const
    {
        ...
        while (pb)
        {
            l += snprintf(&buf[l], sizeof(buf) - l, "%p(H=%d,%d<=%d)-", pb,
                          pb->flags == PBUF_HELPER_FLAG, pb->len, pb->tot_len);
            pb = pb->next;
        }
    ...
    size_t tell() const { return _rx_buf_offset; }
    ...
    bool isValidOffset(const size_t pos) const { return (pos <= _rx_buf_size); }

    netif *getInputNetif() const { return _currentAddr.input_netif; }

    const IPAddress &getRemoteAddress() const { return _currentAddr.srcaddr; }

    uint16_t getRemotePort() const { return _currentAddr.srcport; }

    const IPAddress &getDestAddress() const { return _currentAddr.dstaddr; }
    ...
    bool next()
    {
        ...
        // We have interleaved information on addresses within received pbuf
        // chain: (before ipv6 code we had: (data-pbuf) -> (data-pbuf) ->
        // (data-pbuf) -> ... in the receiving order) Now: (address-info-pbuf ->
        // chained-data-pbuf [-> chained-data-pbuf...]) ->
    ...
    size_t read(char *dst, size_t size)
    {
        ...
        void *buf =
            pbuf_get_contiguous(_rx_buf, dst, size, size, _rx_buf_offset);
        if (!buf)
            return 0;
    ...
    size_t append(const char *data, size_t size)
    {
        ...
        while (left_to_copy)
        {
            ...
            size_t used_cur =
                _tx_buf_offset - (_tx_buf_head->tot_len - _tx_buf_cur->tot_len);
            ...
            memcpy(reinterpret_cast<char *>(_tx_buf_cur->payload) + used_cur,
                   data, will_copy);
                     ...
                     esp8266::polledTimeout::oneShotFastMs::timeType timeoutMs)
    {
        ...
        while (((err = trySend(addr, port, /* keep buffer on error */ true)) !=
                ERR_OK) &&
               !timeout)
            esp_yield();
        if (err != ERR_OK)
  ...
  private:
    ...
    void _recv(udp_pcb *upcb, pbuf *pb, const ip_addr_t *srcaddr, u16_t srcport)
    {
        (void)upcb;
        ...
        if (_rx_buf)
        {
            ...
            pbuf *pb_helper = pbuf_alloc(
                PBUF_RAW, sizeof(AddrHelper) + PBUF_ALIGNER_ADJUST, PBUF_RAM);
            if (!pb_helper)
            {
            ...
            // construct in place
            new (PBUF_ALIGNER(pb_helper->payload))
                AddrHelper(srcaddr, ip_current_dest_addr(), srcport,
    ...
    static void _s_recv(void *arg, udp_pcb *upcb, pbuf *p,
