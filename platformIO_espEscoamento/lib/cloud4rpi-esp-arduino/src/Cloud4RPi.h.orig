...
#define CLOUD4RPI_PRINT(...)                                                   \
    {                                                                          \
        CLOUD4RPI_PRINTER.print(__VA_ARGS__);                                  \
    }
#define CLOUD4RPI_PRINTLN(...)                                                 \
    {                                                                          \
        CLOUD4RPI_PRINTER.println(__VA_ARGS__);                                \
    }
#else
#define CLOUD4RPI_PRINT(...)                                                   \
    {                                                                          \
...
class Cloud4RPi
{
  ...
  public:
    Cloud4RPi(const String &deviceToken, const String &server = MQTT_SERVER,
              int port = MQTT_PORT);
    ~Cloud4RPi();
     ...
     * @param reconnectTimeout - time in ms before trying to connect to the
     * broker again.
     * @return The resulting connection status
     */
    bool ensureConnection(int maxReconnectAttempts = C4R_RETRY_FOREVER,
                          int reconnectTimeout = C4R_RECONNECT_TIMEOUT);
    bool connected();
    bool loop();

    void declareBoolVariable(const String &varName,
                             C4R_BOOL_HANDLER_SIGNATURE = NULL);
    void declareNumericVariable(const String &varName,
                                C4R_NUMERIC_HANDLER_SIGNATURE = NULL);
