...
class CertStoreBase
{
  public:
    ...
    // Installs the cert store into the X509 decoder (normally via static
    // function callbacks)
...
class CertStore : public CertStoreBase
{
  public:
    ...
    int initCertStore(fs::FS &fs, const char *indexFileName,
  ...
  protected:
    ...
    static CertInfo _preprocessCert(uint32_t length, uint32_t offset,
                                    const void *raw);
};
