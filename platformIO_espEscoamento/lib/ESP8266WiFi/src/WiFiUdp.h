...
class WiFiUDP : public UDP, public SList<WiFiUDP>
{
  ...
  public:
    ...
    // port Returns 1 if successful, 0 if there was a problem with the supplied
    // IP address or port
    int beginPacket(IPAddress ip, uint16_t port) override;
    // Start building up a packet to send to the remote host specific in host
    // and port Returns 1 if successful, 0 if there was a problem resolving the
    // hostname or port
    int beginPacket(const char *host, uint16_t port) override;
    // Start building up a packet to send to the multicast address
    // multicastAddress - multicast address to send to
    // interfaceAddress - the local IP address of the interface that should be
    // used
    //                    use WiFi.localIP() or WiFi.softAPIP() depending on the
    //                    interface you need
    ...
    // next byte
    int peek() override;
    void flush() override; // wait for all outgoing characters to be sent,
    // output buffer is empty after this call
