...
inline constexpr auto DNSResolveTypeDefault =
    static_cast<DNSResolveType>(LWIP_DNS_ADDRTYPE_DEFAULT);
...
class ESP8266WiFiGenericClass
{
  ...
  public:
    ...
    // Note: this function is deprecated. Use one of the functions below
    // instead.
    void onEvent(WiFiEventCb cb, WiFiEvent_t event = WIFI_EVENT_ANY)
        __attribute__((deprecated));

    // Subscribe to specific event and get event information as an argument to
    // the callback
    ...
    WiFiEventHandler
        onWiFiModeChange(std::function<void(const WiFiEventModeChange &)>);
     ...
     */
    bool getSleep() { return getSleepMode() == WIFI_MODEM_SLEEP; }
    ...
    static void preinitWiFiOff()
        __attribute__((deprecated("WiFi is off by default at boot, use "
                                  "enableWiFiAtBoot() for legacy behavior")));
  ...
  public:
    ...
    int hostByName(const char *aHostname, IPAddress &aResult,
                   uint32_t timeout_ms, DNSResolveType resolveType);
#endif
    bool getPersistent();
